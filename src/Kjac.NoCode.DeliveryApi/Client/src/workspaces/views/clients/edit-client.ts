...
export default class EditClientModalElement
  ...
  @query('#clientName')
  private _clientNameElement!: UUIInputElement;

  @query('#origin')
  private _originElement!: UUIInputElement;
  ...
  #submit(ev: Event) {
    ...
    if (!this._clientNameElement.checkValidity() || !this._originElement.checkValidity()) {
      ...
      <umb-body-layout headline="${this.data?.headline}">
        <uui-form>
          <uui-box>
            ...
            <uui-label for="origin" required class="spacing-above">Origin</uui-label>
